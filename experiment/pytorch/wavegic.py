...
# -*- coding: UTF-8 -*-
'''=================================================
@Project -> File   ：GraphTools -> wavegic
@IDE    ：PyCharm
@Author ：pengchenghu
...
import config.read_wavegic_config as config
...
# GCN 分类器
class WaveGICClassifier(nn.Module):
    def __init__(self,
                 in_feats,
                 n_output=2,
                 ...
                 dropout=0.5,
                 pooling_type='h'):
        super(WaveGICClassifier, self).__init__()
...
def build_model(in_feats, n_output=2,n_hidden=[256, 256, 256],
    ...
    model = WaveGICClassifier(in_feats=in_feats, n_output=n_output,
...
def main():
    ...
    model_name = "{}bs_{}ep_{}es_{:.5f}lr_{}hi_{:.2f}dp_{}pt_{}fd_{}nat".format(config.batch_size, config.epoch,
                                                                                config.es_patience,
    ...
    for random_seed in config.random_seeds:
        metrics_result = k_fold_train(dataset=graph_dataset, model_fn=build_wave_gic_model, n_fold=config.n_fold,
...
if __name__ == "__main__":
    ...
    print("WaveGic_Classifier_Experiments")
    main()
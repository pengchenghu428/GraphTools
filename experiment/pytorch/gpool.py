...
import config.read_gpool_config as config
...
# GCN 分类器
class GPoolClassifier(nn.Module):
    def __init__(self,
                 in_feats,
                 n_output=2,
                 ...
                 dropout=0.5,
                 pooling_type='h'):
        super(GPoolClassifier, self).__init__()
...
def build_model(in_feats, n_output=2,n_hidden=[256, 256, 256], n_node=[32,16,8],
    ...
    model = GPoolClassifier(in_feats=in_feats, n_output=n_output,
...
def main():
    ...
    for random_seed in config.random_seeds:
        metrics_result = k_fold_train(dataset=graph_dataset, model_fn=build_gpool_model, n_fold=config.n_fold,
...
if __name__ == "__main__":
    ...
    print("GPool_Classifier_Experiments")
    main()
